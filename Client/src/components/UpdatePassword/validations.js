...
export default function validations(inputs) {
  ...
  if (!inputs.clave) {
    errors.clave = "Debe ingresar su nueva clave";
  } else if (!regex.test(inputs.clave)) {
  ...
  if (!inputs.confirmarClave) {
    errors.confirmarClave = "Debe confirmar su nueva clave";
  } else if (!regex.test(inputs.confirmarClave)) {
